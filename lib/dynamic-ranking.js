...
export async function createDynamicRanking(rawData, options) {
    ...
    // validate data
    validate(rawData, options.featureColumns);

    // normalize the data
    const processedData = normalize(rawData, options.normalization, options.featureColumns);
    ...
    renderUI(container, scores, options.featureColumns, options.defaultWeights, processedData, options.model);
}
...
function renderUI(container, scores, featureColumns, defaultWeights, data, model) {
    ...
    for (let i = 0; i < featureColumns.length; i++) {
        ...
        <label for="slider${i}}">${featureColumns[i]}</label>
        ...
        slider.addEventListener('change', function () {
            ...
            const scores = calculateScores(data, featureColumns, model, defaultWeights)
            renderUI(container, scores, featureColumns, defaultWeights, data, model)
        });
